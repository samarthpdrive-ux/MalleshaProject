...
function streamDriveVideo(req, res, id) {
  ...
  const options = {
    ...
    path: `/uc?export=download&id=${encodeURIComponent(id)}&confirm=t`,
  ...
  https.get(options, driveRes => {
    ...
    if (driveRes.statusCode >= 300 && driveRes.statusCode < 400 && driveRes.headers.location) {
      driveRes.resume();
      return https.get(driveRes.headers.location, {headers: req.headers.range ? {Range:req.headers.range} : {}}, redirected => {
